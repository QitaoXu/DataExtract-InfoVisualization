...
do
    ...
    do
        ...
        sim_command=`$GEM5/build/X86/gem5.opt --outdir=${file_dir}, -d ${file_dir} hw3config.py -c daxpy/daxpy --cpu-type="DerivO3CPU" --caches --l2cache --num-phys-float-regs=${num_phys_float_regs} --num-rob-entries=${num_rob_entries} --num-iq-entries=${num_iq_entries}`
        echo "\n"
        echo $sim_command
        echo "\n"
